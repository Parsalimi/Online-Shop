...
def ShowMainMenu():
    UsersUpdate()
    with open("extra_projects\\online-shop\\DB\\theLatestLoginUserId.txt","r") as file:
...
def SearchUser(username, password, id): 
    for user in Main.users:
        if (user[1] == username and user[2] == password) or user[0] == id:
            ...
            with open("extra_projects\\online-shop\\DB\\theLatestLoginUserId.txt","w") as file:
                file.write(user[0])
                file.close()
...
def UsersUpdate():
    file = open("extra_projects\\online-shop\\DB\\users.txt","r")
...
def SignUpMenu():    
    ...
    with open("extra_projects\\online-shop\\DB\\users.txt","a") as file:
...
def AddItemMenu():
    ...
    if Main.selectedUser.role == "2":
        ...
        while flag:
            ...
            isDiscounted = "0"
            discountDate = ""
            with open("extra_projects\\online-shop\\DB\\item.txt", "w") as file:
...
def ShowItems():
    if Main.selectedUser.role == "2": # if admin enters
        with open("extra_projects\\online-shop\\DB\\item.txt", "r") as file:
            fileText = file.read()
